...
class navigation_demo:
    ...
    def __init__(self):
        ...
        # 用定时器定时查询TF（20Hz），替代原来的回调方式
        self.tf_timer = rospy.Timer(rospy.Duration(0.05), self.tf_pose_cb)
        self.current_x = 0.0
        self.current_y = 0.0
        self.current_yaw = 0.0
    ...
    def tf_pose_cb(self, event):
        try:
            # 监听 map -> base_footprint 的坐标变换（也可用 base_link，按你实际tf树调整）
            (trans, rot) = self.tf_listener.lookupTransform('/map', '/base_footprint', rospy.Time(0))
            self.current_x = trans[0]
            self.current_y = trans[1]
            # 四元数转欧拉角，获取偏航角
            r, p, yaw = transformations.euler_from_quaternion(rot)
            self.current_yaw = yaw
            # 把最新位姿传给DockPID
            self.dock_pid.update_pose(self.current_x, self.current_y, self.current_yaw)
        except (tf.LookupException, tf.ConnectivityException, tf.ExtrapolationException):
            rospy.logwarn_throttle(5, "TF lookup failed, using last known pose")
    ...
    def pid_goto(self,pid_g):
        ...
        if pid_g == 1:
            ...
            self.dock_pid.precise_dock(target_x=1.10, target_y=-0.40, target_yaw_deg=0)
            case = 0
            #return True
        ...
        if pid_g == 2:
            ...
            self.goto_x('on')
            print(self.current_x, self.current_y, self.current_yaw)
            self.dock_pid.precise_dock(target_x=1.10, target_y=-1.60, target_yaw_deg=0.00)
            case = 1
            #return True
        ...
        if pid_g == 3:
            ...
            self.goto_x('on')
            print(self.current_x, self.current_y, self.current_yaw)
            self.dock_pid.precise_dock(target_x=1.10, target_y=-2.75, target_yaw_deg=0.00)
            case = 2
            #return True
    ...
    def find_cb(self, data):
        ...
        if abs(flog1) > 5 and point_msg.z == 51 and flog2 >= 255 and case == 0:
            print('瞄准中[马了]')
            # 初始化速度消息
            msg = Twist()
        ...
        elif abs(flog1) <= 5 and point_msg.z == 51 and flog2 >= 255 and case == 0:
            # 串口发送射击启动指令
            #ser.write(b'\x55\x01\x12\x00\x00\x00\x01\x69')
            print("发射[好枪兄弟]")
            rospy.sleep(0.08)
...
if __name__ == "__main__":
    ...
    if input == '1':
        ...
        #rospy.sleep(2)
        navi.dock_pid.precise_dock(target_x=1.10, target_y=-0.40, target_yaw_deg=0)
